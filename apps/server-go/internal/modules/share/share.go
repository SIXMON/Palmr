...
func (h *Handler) ListMine(ctx context.Context, _ *struct{}) (*ShareListOutput, error) {
	...
	rows, err := h.DB.QueryxContext(ctx, `SELECT id FROM shares WHERE creatorId = ? ORDER BY createdAt DESC`, uc.UserID)
	if err != nil {
	...
	for rows.Next() {
		var id string
		_ = rows.Scan(&id)
		if v, err := h.fullView(ctx, id); err == nil {
			out.Body.Shares = append(out.Body.Shares, v)
		}
	}
	return out, nil
}
...
func (h *Handler) AddItems(ctx context.Context, in *ShareItemsInput) (*ShareGetOutput, error) {
	...
	for _, fid := range in.Body.Files {
		_, _ = h.DB.ExecContext(ctx, `INSERT OR IGNORE INTO _ShareFiles (A, B) VALUES (?, ?)`, fid, in.ShareID)
	}
	for _, fid := range in.Body.Folders {
		_, _ = h.DB.ExecContext(ctx, `INSERT OR IGNORE INTO _ShareFolders (A, B) VALUES (?, ?)`, fid, in.ShareID)
	}
	v, _ := h.fullView(ctx, in.ShareID)
	out := &ShareGetOutput{}
	out.Body.Share = v
...
func (h *Handler) RemoveItems(ctx context.Context, in *ShareItemsInput) (*ShareGetOutput, error) {
	...
	for _, fid := range in.Body.Files {
		_, _ = h.DB.ExecContext(ctx, `DELETE FROM _ShareFiles WHERE A = ? AND B = ?`, fid, in.ShareID)
	}
	for _, fid := range in.Body.Folders {
		_, _ = h.DB.ExecContext(ctx, `DELETE FROM _ShareFolders WHERE A = ? AND B = ?`, fid, in.ShareID)
	}
	v, _ := h.fullView(ctx, in.ShareID)
	out := &ShareGetOutput{}
	out.Body.Share = v
...
func (h *Handler) Notify(ctx context.Context, in *NotifyInput) (*ShareMsgOutput, error) {
	uc, err := auth.EnsureAuth(ctx)
	if err != nil {
		return nil, apperr.Unauthorized(err.Error())
	}
	...
	// TODO: wire to email service once that lands. For now we acknowledge
	// the request so the UI flow completes.
	out := &ShareMsgOutput{}
...
func (h *Handler) GetByAlias(ctx context.Context, in *ByAliasInput) (*ByAliasOutput, error) {
	...
	// Increment views
	_, _ = h.DB.ExecContext(ctx, `UPDATE shares SET views = views + 1 WHERE id = ?`, shareID)
...
func (h *Handler) GetAliasMetadata(ctx context.Context, in *ShareAliasMetaInput) (*ShareAliasMetaOutput, error) {
	...
	_ = row.Scan(&out.Body.Name, &out.Body.Description)
